...
tk.Button(input_frame, text="Add Number", command=Finale.add_number,
          bg="#fe7f2d", fg="white",
...
for i, (text, mode) in enumerate(btn_configs):
    tk.Button(ops_frame, text=text, width=12,
              command=lambda m=mode: Finale.perform_op(m),
              bg="#fe7f2d", fg="white",
...
tk.Button(rot_frame, text="Rotate Last Number", command=Finale.rotate_selected,
...
tk.Button(scale_frame, text="Scale Last Number", command=Finale.scale_selected,
          bg="#fe7f2d", fg="white", font=("Segoe UI", 9, "bold")).grid(row=0, column=2, padx=10)
...
tk.Button(scrollable_frame, text="DISPLAY ARGAND PLOT", command=Finale.show_plot,
          bg="#fe7f2d", fg="black",
          activebackground="#f4a261", activeforeground="black",
          font=("Segoe UI", 11, "bold"), height=2, relief="flat").pack(fill="x", padx=40, pady=5)

tk.Button(scrollable_frame, text="RESET ALL DATA", command=Finale.clear_all,
          bg="#fe7f2d", fg="black",
          activebackground="#f4a261", activeforeground="black",
          font=("Segoe UI", 9, "bold"), relief="flat", width=20).pack(pady=10)

# Connect GUI widgets to logic
Finale.entry_real = entry_real
Finale.entry_imag = entry_imag
Finale.listbox_numbers = listbox_numbers
Finale.lbl_result = lbl_result
Finale.entry_angle = entry_angle
Finale.entry_scale = entry_scale
